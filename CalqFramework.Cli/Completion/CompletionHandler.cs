...
public class CompletionHandler : ICompletionHandler {
    ...
    private void HandleComplete(ICliContext context, object target, List<string> args, string toComplete) {
        ...
        if (toComplete.StartsWith("-")) {
            ...
            Parameter? parameter = subcommandExecutorWithOptions.GetParameters()
                .FirstOrDefault(p => p.Keys.Contains(optionName));
            if (parameter != null) {
                CompletionPrinter.PrintParameterValue(context, parameter, toComplete, submodule);
            } else {
                Option? option = subcommandExecutorWithOptions.GetOptions()
                    .FirstOrDefault(o => o.Keys.Contains(optionName));
                if (option != null) {
                    CompletionPrinter.PrintOptionValue(context, option, toComplete, submodule);
                }
            }
        } else {
            Parameter? parameter = subcommandExecutorWithOptions.GetFirstUnassignedParameter();
