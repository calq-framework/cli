...
public class CommandLineInterfaceTest {
    ...
    public void Execute_MethodWithCorrectCommandName_ExecutesSuccessfully() {
        ...
        new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.Method))}".Split(' '));
    }
    ...
    public void Execute_MethodWithLowercaseCommandName_ExecutesSuccessfully() {
        SomeClassLibrary tool = new();
        new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.Method)).ToLower()}".Split(' '));
    }
    ...
    public void Execute_MethodWithOptionalParameter_OmitsOptionalParam() {
        SomeClassLibrary tool = new();
        new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithOptionalParam))}".Split(' '));
    }
    ...
    public void Execute_ObjectFieldWithNestedCommand_ExecutesNestedMethod() {
        SomeClassLibrary tool = new();
        new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.objectField))} nested-method".Split(' '));
    }
    ...
    public void Execute_MethodWithHelpOption_GeneratesMethodHelp() {
        SomeClassLibrary tool = new();
        new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.Method))} --help".Split(' '));
    }
    ...
    public void Execute_MethodWithListParameter_AppendsMultipleValues() {
        ...
        object result = new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithList))} --param-list false --param-list true"
                .Split(' '));
    ...
    public void Execute_MethodWithSetParameter_DeduplicatesValues() {
        SomeClassLibrary tool = new();
        object result = new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithSet))} --tags urgent --tags important --tags urgent"
                .Split(' '));
    ...
    public void Execute_MethodWithInitializedBoolList_UpdatesListElements() {
        SomeClassLibrary tool = new();
        object result = new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.Method))} --{StringHelper.GetKebabCase(nameof(SomeClassLibrary.initializedBoolList))} false --{StringHelper.GetKebabCase(nameof(SomeClassLibrary.initializedBoolList))} true"
                .Split(' '));
    ...
    public void Execute_MethodWithNamedInteger_ReturnsIntegerResult() {
        SomeClassLibrary tool = new();
        object result = new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithInteger))} --integer 1".Split(' '));
    ...
    public void Execute_MethodWithPositionalParameter_BindsCorrectly() {
        SomeClassLibrary tool = new();
        object result = new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithInteger))} 1".Split(' '));
        Assert.Equal(1, result);
    }
    ...
    public void Execute_MethodWithNamedText_ReturnsTextValue() {
        SomeClassLibrary tool = new();
        object result = new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithText))} --text abc".Split(' '));
    ...
    public void Execute_MethodWithPositionalText_ReturnsTextValue() {
        SomeClassLibrary tool = new();
        object result = new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithText))} abc".Split(' '));
        Assert.Equal("abc", result);
    }
    ...
    public void Execute_MethodWithConflictingBooleanOption_ThrowsException() {
        CliException ex = Assert.Throws<CliException>(() => {
            SomeClassLibrary tool = new();
            new CommandLineInterface().Execute(tool,
                $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithTextAndBooleanError))} abc --{StringHelper.GetKebabCase(nameof(SomeClassLibrary.booleanConflict))}"
                    .Split(' '));
        });
    }
    ...
    public void Execute_InvalidFieldCommand_ThrowsCliExceptionWithMessage() {
        CliException ex = Assert.Throws<CliException>(() => {
            SomeClassLibrary tool = new();
            new CommandLineInterface().Execute(tool,
                $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.textField))}".Split(' '));
        });
    ...
    public void Execute_MethodWithMixedParameterOrder_ExecutesSuccessfully() {
        SomeClassLibrary tool = new();
        object result = new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithIntegerAndText))} abc --integer 1"
                .Split(' '));
    }
    ...
    public void Execute_MethodWithNamedIntegerBeforePositionalText_ExecutesSuccessfully() {
        SomeClassLibrary tool = new();
        object result = new CommandLineInterface().Execute(tool,
            $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithTextAndInteger))} --integer 1 abc"
                .Split(' '));
    }
    ...
    public void Execute_MethodWithInvalidParameterOrder_ThrowsCliException() {
        CliException ex = Assert.Throws<CliException>(() => {
            SomeClassLibrary tool = new();
            new CommandLineInterface().Execute(tool,
                $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.Method))} abc".Split(' '));
        });
    }
    ...
    public void Execute_MethodWithMissingRequiredParameter_ThrowsCliException() {
        CliException ex = Assert.Throws<CliException>(() => {
            SomeClassLibrary tool = new();
            new CommandLineInterface().Execute(tool,
                $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.MethodWithText))}".Split(' '));
        });
    ...
    public void Execute_NullObjectFieldWithNestedCommand_ThrowsNullReferenceException() {
        NullReferenceException ex = Assert.Throws<NullReferenceException>(() => {
            SomeClassLibrary tool = new();
            new CommandLineInterface().Execute(tool,
                $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.nullObjectField))} mested-method".Split(' '));
        });
    }
    ...
    public void Execute_UnknownOptionWithValue_ThrowsUnknownOptionException() {
        CliException ex = Assert.Throws<CliException>(() => {
            SomeClassLibrary tool = new();
            new CommandLineInterface().Execute(tool,
                $"{StringHelper.GetKebabCase(nameof(SomeClassLibrary.Method))} --doesntexist a".Split(' '));
        });
