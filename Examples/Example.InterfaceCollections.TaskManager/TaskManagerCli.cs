...
public class TaskManagerCli {
    ...
    /// <returns>Result message.</returns>
    public string AddTags(ICollection<string> taskNames, ISet<string> tags) => $"Added tags [{string.Join(", ", tags)}] to {taskNames.Count} tasks: {string.Join(", ", taskNames)}";
    ...
    /// <returns>Result message.</returns>
    public string UpdateStatus(IReadOnlyList<string> taskNames, TaskStatus status) => $"Updated {taskNames.Count} tasks to status: {status}";
    ...
    /// <returns>Result message.</returns>
    public string GetByTags(IReadOnlyCollection<string> tags) => $"Finding tasks with tags: {string.Join(", ", tags)} (searching {tags.Count} tags)";
    ...
    /// <returns>Result message.</returns>
    public string BulkUpdate(IList<string> tasks, ICollection<string> addTags) => $"Bulk updated {tasks.Count} tasks with {addTags.Count} tags";

    /// <summary>Search tasks by keywords (concrete List type).</summary>
    /// <param name="keywords">Keywords to search for.</param>
    /// <returns>Result message.</returns>
    public string Search(List<string> keywords) => $"Searching for tasks matching: {string.Join(", ", keywords)}";

    /// <summary>Filter by statuses (concrete List with enum).</summary>
    /// <param name="statuses">Statuses to filter by.</param>
    /// <returns>Result message.</returns>
    public string FilterByStatus(List<TaskStatus> statuses) => $"Filtering tasks by statuses: {string.Join(", ", statuses)}";
