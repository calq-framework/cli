...
public class CloudProviderCli {
    ...
    public string ConfigureRegion([CliCompletion(typeof(RegionCompletionProvider))] string region) => $"[{Verbosity}] Configured region: {region} for profile '{Profile}' (format: {Format})";
    ...
    public string Backup([CliCompletion(typeof(DirectoryCompletionProvider))] string outputDir) => $"[{Verbosity}] Backed up configuration to {outputDir} (format: {Format})";
    ...
    public string Import([CliCompletion(typeof(FileSystemCompletionProvider), "*.json;*.yaml")] string path) => $"[{Verbosity}] Imported configuration from {path} (format: {Format})";
    ...
    public string ReadLog(System.IO.FileInfo logFile) => $"[{Verbosity}] Reading log file: {logFile.FullName} (format: {Format})";
    ...
    public string ListResources(System.IO.DirectoryInfo resourceDir) => $"[{Verbosity}] Listing resources in: {resourceDir.FullName} (format: {Format})";
